...
void PlayingState::DrawRangeIndicator(Game& game) {
    ...
    // Show range of whichever placed tower the mouse is over
    Vector2 mouseWorld = game.GetInput().GetWorldMousePosition(m_renderSystem.GetCamera());
    int hx, hy;
    if (game.GetGameData().map.WorldToTile(mouseWorld, hx, hy)) {
        const Tile& tile = game.GetGameData().map.Get(hx, hy);
        if (tile.m_towerKey != DenseSlotMap<Tower>::INVALID_KEY) {
            if (const Tower* t = game.GetGameData().towers.Get(tile.m_towerKey))
                m_renderSystem.DrawTowerRange(t->m_position, t->m_radius, {255, 255, 255, 80});
        }
    }
}

void PlayingState::DrawGhostTower(Game& game) {
    if (game.GetGameData().waveActive) return;
    if (m_selectedTowerKey != DenseSlotMap<Tower>::INVALID_KEY) return;
